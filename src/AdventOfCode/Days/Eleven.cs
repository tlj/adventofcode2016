...
namespace Days
{
    public class AShittyClass
    {
        ...
        public AShittyClass(int e, List<int[]> p, List<string> l, int d)
        {
            elevatorFloor = e;
            pairs = p;
            log = l;
    ...
    public class Eleven : Base
    {
        ...
        public List<List<int[]>> GetValidStates(List<int[]> pairs, int currentFloor, int newFloor, List<string> log)
        {
            ...
            for (var i = 0; i < pairs.Count; i++) {
                if (pairs[i][0] == currentFloor) {
                    ...
                    if (Validate(clonedPairs, currentFloor, newFloor) && !log.Contains(Normalize(clonedPairs))) {
        ...
        public bool Solve(AShittyClass sc)
        {
            ...
            foreach (var elevatorChange in upDown) {
                ...
                //Console.WriteLine("OldFloor: " + sc.elevatorFloor + " New floor: " + newFloor);

                var validStates = GetValidStates(sc.pairs, sc.elevatorFloor, newFloor, sc.log);
                if (validStates.Count == 0) {
                    //Console.WriteLine("Ending branch after " + depth + " depth.");
                }
                foreach (var vs in validStates) {  
                    sc.log.Add(Normalize(vs));
                    //Console.WriteLine(Normalize(vs) + " E" + newFloor);
                    if (IsSolved(vs)) {
                        if (sc.depth < lowestCount) {
                            lowestCount = sc.depth;
                            Console.WriteLine("New lowest: " + lowestCount);
        ...
        public override void Run()
        {
            ...
            Solve(new AShittyClass(0, pairs, new List<string>(), 1));

            while (possibleStates.Count > 0 && !Solve(possibleStates.Dequeue())) {
                //Console.WriteLine(possibleStates.Count);
            }
            Console.WriteLine("Checked " + checkedStates + " states.");
        }
